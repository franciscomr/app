      ...
      <div class="max-w-md mx-auto">
        <div class="flex flex-col bg-white p-6 dark:bg-white/10 space-y-4 rounded-t-md">
          ...
          <div v-for="(relationship) in relationships">
            ...
            v-model="form.data.relationships[relationship.id]" :error-message="errors[relationship.id]" />
          </div>
          
          <div v-for="(attribute) in attributes">

            <FormInput v-if="attribute.type === 'input'" :id="attribute.id" :label="attribute.name" :type="attribute.inputType" 
            v-model="form.data.attributes[attribute.id]" :error-message="errors[attribute.id]" />
  ...
  setup(props) {
    ...
    onMounted(async() => {
      if (props.formAction === 'edit') {
        ...
        form.value.data.attributes = singleRecord.data.attributes
